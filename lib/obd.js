    ...
    this.intervalWriter = setInterval(function () {
        if (queue.length > 0 && self.connected)
            try {
					...
					if(debug){
						console.log("Sending: " + new Buffer(queue[0], "utf-8"));
					}
...
OBDReader.prototype.testMsg = function (msg) {

		if(debug){
			console.log("Sending: " + new Buffer(msg, "utf-8"));
		}
		
    this.btSerial.write(new Buffer(msg, "utf-8"), function (err, count) {
		if (err)
			self.emit('error', err);
	});
};
...
OBDReader.prototype.listPollers = function () {
    console.log(JSON.stringify(activePollers,null,2));
};
